...
class TinyNode(ABC):
    ...
    def __init_subclass__(cls, **kwargs):
        ...
        Registers created class in TinyNode._lookup_statement_type.
        """
        TinyNode._lookup_statement_type[cls.statement_type] = cls
    ...
    ) -> object | None:  # noqa: F821 - forward ref
        """Execute this node against the given engine.

        Subclasses must implement this method.
        """
        raise NotImplementedError(
            f"execute() not implemented for {type(self).__name__}"
        )
...
class IfStmtNode(TinyNode):
    ...
    def from_parsed(cls, parsed: pp.ParseResults) -> IfStmtNode:
        ...
        branches.append((parsed.cond, cls.body_statements(parsed.then)))
